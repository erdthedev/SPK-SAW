...
<div class="card shadow mb-4">
    ...
    <div class="card-body">
		<?= form_open('penilaian/update') ?>
			<?= form_hidden('id_alternatif', $alternatif->id) ?>
			<div class="form-row">
				<?php foreach($kriteria as $data): ?>
				<?php
					$nilai = isset($penilaian[$data->id]) ? $penilaian[$data->id]->nilai : 0;
					$id_penilaian = isset($penilaian[$data->id]) ? $penilaian[$data->id]->id_penilaian : '';
				?>
				<div class="form-group col-md-4">
					<label for="" class="text-dark"><?=  $data->kode_kriteria . ' - ' . $data->nama_kriteria ?></label>
					<input type="number" class="form-control" id="" placeholder="" autocomplete="off" required name="<?= $data->kode_kriteria ?>" value="<?= $nilai ?>">
					<?= form_hidden('id_penilaian_'.$data->kode_kriteria, $id_penilaian) ?>
				</div>
				<?php endforeach; ?>
			</div>
